...
#like public methods
    ...
    #Calc methods
    def New_CalcKoefficients(self, functionWithConformity):
        calcMatrix_ = self.New_InitializeCalcMatrix_(functionWithConformity);
    ...
    def New_CalcDiscripancy(self, koefficients, functionWithConformity):
        """
        Вычисление невязки
        """
        height_ = len(self.parameters);
        ...
        for rowIdx in range(0, height_):
            ...
            for funcIdx in range(len(functionWithConformity)):
                sum += koefficients[funcIdx] * functionWithConformity[funcIdx](self.GetFunctionParameters_(rowIdx, functionWithConformity[funcIdx].conformity));
            disc = self.results[rowIdx] - sum;
            squareDiscripancySum += disc * disc;
...
#like protected methods
    ...
    def New_InitializeCalcMatrix_(self, functionWithConformity):
        calcMatrix_ = [];
        ...
        for funcIdx1 in range(len(functionWithConformity)):
            ...
            for funcIdx2 in range(len(functionWithConformity)):
                colomnSum = 0;
                for rowIdx in range(0, len(self.parameters)):
                    colomnSum += functionWithConformity[funcIdx1](self.GetFunctionParameters_(rowIdx, functionWithConformity[funcIdx1].conformity)) * \
                        functionWithConformity[funcIdx2](self.GetFunctionParameters_(rowIdx, functionWithConformity[funcIdx2].conformity));
                row.append(colomnSum);

            colomnSum = 0;
            for rowIdx in range(0, len(self.parameters)):
                colomnSum += self.results[rowIdx] * functionWithConformity[funcIdx1](self.GetFunctionParameters_(rowIdx, functionWithConformity[funcIdx1].conformity));	
            row.append(colomnSum);
            calcMatrix_.append(row);
    ...
    def GetFunctionParameters_(self, rowIdx, conformite):
        ...
        for i in range(0, len(conformite)):
           row.append(self.parameters[rowIdx][conformite[i]]);
